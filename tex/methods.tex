Previous implementations of \pygbe use continuum electrostatic to compute the
solvation energy of proteins with multiple dielectric regions \cite{CooperBardhanBarba2013}, and the
interaction between a protein and a charged surface \cite{CooperClementiBarba2015}. We solved the Poisson-Boltzmann
equation of the implicit-solvent model in its integral form obtained by following
mathematical the formulation of Yoon and Lenhoff \cite{YoonLenhoff1990}. This paper
...
To perform response calculations of LSPR biosensors using our electrostatics we
re-wrote our Maxwell equations to include the effect of an incoming electric 
field, their potential formulation and finally derived an integral formulation 
that is appropriate for the new physics. 
...
of boundary integral equations \cite{BrebbiaDominguez1992}. Evaluating on the surface $\Gamma$, the system
becomes
%
...
For the system in Figure \ref{fig:analyte-sensor} the potential formulation, can
be written as:
...
Writing euqations \eqref{eq:electrostatic_scatter_prot_sen} as a system 
...
and equation \eqref{eq:integral_eq_lspr_nobc_system} can be represented as:
