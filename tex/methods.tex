...
\textcolor{red}{Chris shouldn't the $d\Gamma$ have a prime?}
...
If we write one component, let's say $p_i$,
...
\text{applying Divergence theorem} \qquad \qquad \qquad \qquad & \nonumber \\
= \oint_\Gamma  x_i  \nabla \phi_{2s} \cdot \mathbf{n} \; \text{d}\Gamma - \int_\Omega \nabla x_i \cdot \nabla\phi_{2s} \; \text{d}\Omega \quad & \nonumber\\
\end{align}

Using identity \eqref{eq:identity_grad} in equation \eqref{eq:dip_gauss_interm_1}, by selecting
...
\text{applying Divergence theorem} \qquad \qquad \qquad \qquad & \nonumber \\
= \oint_\Gamma  x_i  \frac{\partial \phi_{2s}}{\partial n} \; \text{d}\Gamma - \oint_\Gamma \phi_{2s} \nabla x_i \cdot \mathbf{n} \; \text{d}\Gamma \qquad \qquad \qquad \qquad & \nonumber \\
= \oint_\Gamma  x_i  \frac{\partial \phi_{2s}}{\partial n} \; \text{d}\Gamma - \oint_\Gamma \phi_{2s} n_i \;\text{d}\Gamma \qquad \qquad \qquad \qquad \qquad
\end{align}

All this normal vectors in the above derivation point from $\Omega_2$ to $\Omega_1$, in other 
words, they point inside $\Omega_1$. Since the code implementation requires all normal vectors
pointing outwards the final equation for the boundary integral formulation of one component of 
the dipole reduces to:
...
and we use it in equation \eqref{eq:dipole_def_gauss_i_final} to get the dipole 
moment, and in equation \eqref{eq:scat_efield_long_range} to obtain the scattered
electric field. Then, we can use the scattered electric field to get scattering 
amplitud using equation \eqref{eq:scat_efield_fwa} which we plugged into \eqref{eq:cext_fwa}
to get the extinction cross-section.
