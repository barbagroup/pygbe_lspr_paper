...
For example, BEM++ \cite{SmigajETal2015} would also model the system as a set of boundary integral equatons, discretized in flat triangular panels. 
However, the Galerkin approach and acceleration with hierarchical matrices produces a method that is slower and less memory efficient, which limits the accessible problem sizes.
The Matlab toolbox MNPBEM \cite{HohenesterTrugler2012} is another alternative which was designed to simulate scattering of metallic nanoparticles.
MNPBEM is similar to \pygbe as it uses a centroid collocation scheme on flat triangular panels, but differs in the algorithmic acceleration technique, which is also based on hierarchical matrices, rather than the treecode. 
This results in higher memory usage compared to our code, making it hard to simulate large analytes in detail.
Other finite-element or finite-difference based solvers could also be used in this application, for example, COMSOL. 
These volumetric approaches, however, struggle to correctly impose the boundary condition equal to zero at infinity, which is exactly met for a BEM formulation.
