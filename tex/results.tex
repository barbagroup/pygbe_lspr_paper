...
plasmon resonance frequency shifts due to the presence of the analyte.
In this section, we use our BEM approach to
to model the response of LSPR biosensors.

To guarantee an accurate numerical solution, we performed a grid 
convergence analysis of the system sketched by Figure \ref{fig:setup_conv}, with
the Bovine Serum Albumina (BSA, PDB code 4FS5) protein as analyte. The BSA mesh
was generated using the open source software Nanoshaper \textcolor{red}{(missing how to cite - not
reported in their webpage)}. Nanoshaper takes as input the atomic coordinates and radii, 
...
cases we decided to use a mesh with density of two triangles per ${\AA}^2$ for 
