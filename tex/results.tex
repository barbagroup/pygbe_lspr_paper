...
Table \ref{table:quadparams1} lists the Gauss quadrature points used for each type of element. 
...
\textcolor{red}{CITE REPRO-PACKS WHEN READY}
We used a mesh with $N=32768$, and relaxed some parameters compared with the grid convergence results shown previously, yielding errors below $1\%$.
The parameters are shown in Tables \ref{table:quadparams2} and \ref{table:treeparams2}.
...
${\AA}^2$ ($N_{prot}=45140$) and the sphere mesh had $N_{sensor}=32768$. 
...
The run time for each one of these cases was approximately $7.5$ min using one NVIDIA Tesla K40 GPU. 
