"""
To run this case you need to create the corresponding problem folders with the
...
#silver
l_w, er_w, ei_w = numpy.loadtxt('../../data/wave_cext_d_prot_sensor/wave_water_diel_ang_3822.5-3867.5-.5.txt',
                                unpack=True)
l_s, er_s, ei_s = numpy.loadtxt('../../data/wave_cext_d_prot_sensor/wave_silver_diel_ang_3822.5-3867.5-.5.txt',
                                unpack=True)
l_p, er_p, ei_p = numpy.loadtxt('../../data/wave_cext_d_prot_sensor/wave_prot_diel_ang_3822.5-3867.5-.5.txt',
                                unpack=True)

'''
l_w, er_w, ei_w = numpy.loadtxt('../../data/wave_cext_d_prot_sensor/wave_water_diel_ang_half.txt',
                                  unpack=True)
l_s, er_s, ei_s = numpy.loadtxt('../../data/wave_cext_d_prot_sensor/wave_silver_diel_ang_half.txt',
                                  unpack=True)
l_p, er_p, ei_p = numpy.loadtxt('../../data/wave_cext_d_prot_sensor/wave_prot_diel_ang_half.txt',
                                  unpack=True)
'''
'''
#gold 
l_w, er_w, ei_w = numpy.loadtxt('../../data/gold_data/wave_water_diel_ang_5172.5-5272.5-5.txt',
                                unpack=True)
l_s, er_s, ei_s = numpy.loadtxt('../../data/gold_data/wave_gold_diel_ang_5172.5-5272.5-5.txt',
                                unpack=True)
l_p, er_p, ei_p = numpy.loadtxt('../../data/gold_data/wave_prot_diel_ang_5172.5-5272.5-5.txt',
                                unpack=True)
'''
...
numpy.savetxt('../../data/R8/BSA_sensorR80_d=infty_half_ef0.0037.txt', 
...
E_field = []
for lst in e_list:
    E_field.append(lst+[lst[-1]]*1)  #works for 2 protein
#    E_field.append(lst+[lst[-1]]*2)  #works for 3 protein
...
for path in distance_path_folders:
    ...
    numpy.savetxt('../../data/R8/'+path+'_half.txt', 
              list(zip(wave, Cext)),
              fmt = '%.1f %.8f', 
              header = 'lambda [Ang], Cext'+path)
toc_d = time.time()

with open('../../data/R8/time_wave_Cext_R8_silver_half_d=0.1.txt', 'w') as f:
