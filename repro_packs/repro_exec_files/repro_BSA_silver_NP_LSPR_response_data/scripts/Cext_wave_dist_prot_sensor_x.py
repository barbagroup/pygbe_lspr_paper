...
def main(argv=sys.argv):
    '''Creates lspr response data for  BSA_sensorR8_2px_d=1_00
    ...
    numpy.savetxt('../Cext_two_bsa_x_results/BSA_sensorR8_d=infty_3820-3870ang.txt', 
    ...
    distance_path_folders = ['BSA_sensorR8_2px_d=1_00']
    ...
    for path in distance_path_folders:
        ...
        numpy.savetxt('../Cext_two_bsa_x_results/'+path+'_3820-3870ang.txt', 
                  list(zip(wave, Cext)),
                  fmt = '%.1f %.8f', 
                  header = 'lambda [Ang], Cext_'+path)
    toc_d = time.time()

    with open('../Cext_two_bsa_x_results/Time_Cext_variation_with_distance_two_bsa_x.txt', 'w') as f:
