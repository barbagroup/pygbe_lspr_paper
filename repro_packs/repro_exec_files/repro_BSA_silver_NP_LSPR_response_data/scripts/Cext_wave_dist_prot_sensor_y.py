...
def main(argv=sys.argv):
    '''Creates lspr response data for:
     BSA_sensorR8_2py_d=1_00 and for the case of no protein BSA_sensorR8_d=infty. 
    ...
    numpy.savetxt('../Cext_two_bsa_y_results/BSA_sensorR8_d=infty_3820-3870ang.txt', 
    ...
    distance_path_folders = ['BSA_sensorR8_2py_d=1_00']
    ...
    for path in distance_path_folders:
        ...
        numpy.savetxt('../Cext_two_bsa_y_results/'+path+'_3820-3870ang.txt', 
                  list(zip(wave, Cext)),
                  fmt = '%.1f %.8f', 
                  header = 'lambda [Ang], Cext_'+path)
    toc_d = time.time()

    with open('../Cext_two_bsa_y_results/Time_Cext_variation_with_distance_two_bsa_y.txt', 'w') as f:
